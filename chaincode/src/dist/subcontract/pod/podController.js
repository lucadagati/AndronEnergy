...
let PodCrudOperations = class PodCrudOperations extends contractExtension_1.ContractExtension {
        ...
        const pod = [
            {
                podId: "Pod1",
                plantIds: ["Plant2"],
                userConsumptionIds: ["User1", "User2"],
    ...
    async podUpdatePlant(ctx, param) {
        ...
        if (!plant) {
            ...
            if (exist.plantIds.indexOf(params.plantId) !== -1)
                ...
                await plantClass.addPodtoPlant(ctx, JSON.stringify(obj));
    ...
    async removePlantfromPod(ctx, param) {
        ...
        const exist = await this.get(ctx, params.podId);
        if (!exist) {
            throw new Error(`The pod ${params.plantId} does not exist`);
        }
        else if (!plant) {
            ...
            const obj = { podId: params.podId, plantId: params.plantId };
            await plantClass.removePodfromPlant(ctx, JSON.stringify(obj));
    ...
    async podUpdateUsers(ctx, param) {
        const params = JSON.parse(param);
        const userClass = new userConsumptionController_1.UserConsumptionsOperations();
        const exist = await this.get(ctx, params.podId);
        const user = await userClass.get(ctx, params.userConsumptionId);
        if (!user) {
    ...
    async podDeleteUsers(ctx, param) {
        const params = JSON.parse(param);
        const userClass = new userConsumptionController_1.UserConsumptionsOperations();
        const exist = await this.get(ctx, params.podId);
        const user = await userClass.get(ctx, params.userConsumptionId);
        if (!user) {
            ...
            if (exist.userConsumptionIds.indexOf(params.userConsumptionId) !== -1)
                ...
                exist.userConsumptionIds = exist.userConsumptionIds.filter((elem) => elem !== params.userConsumptionId);
                return Promise.all([await ctx.stub.putState('pod-' + exist.podId, Buffer.from(JSON.stringify(exist)))])
                    .then(() => { return { status: asset_1.Status.Success, message: "Operazione effetuata" }; });
            }
        }
...
__decorate([
    ...
    __metadata("design:paramtypes", [fabric_contract_api_1.Context, String]),
    __metadata("design:returntype", Promise)
], PodCrudOperations.prototype, "podUpdateUsers", null);
__decorate([
    fabric_contract_api_1.Transaction(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [fabric_contract_api_1.Context, String]),
    __metadata("design:returntype", Promise)
], PodCrudOperations.prototype, "podDeleteUsers", null);
__decorate([
    fabric_contract_api_1.Transaction(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [fabric_contract_api_1.Context, String]),
