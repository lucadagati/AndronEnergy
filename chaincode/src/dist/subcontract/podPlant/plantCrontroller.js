...
let PlantOperations = class PlantOperations extends contractExtension_1.ContractExtension {
    ...
    async addPodtoPlant(ctx, param) {
        const params = JSON.parse(param);
        const podClass = new podController_1.PodCrudOperations();
        const pod = await podClass.get(ctx, params.podId);
        const plant = await this.get(ctx, params.plantId);
        if (plant.plantId == undefined) {
            throw new Error("The plant  with id:" + plant.plantId + " does not exists");
        }
    ...
    async removePodfromPlant(ctx, param) {
        const params = JSON.parse(param);
        const podClass = new podController_1.PodCrudOperations();
        const pod = await podClass.get(ctx, params.podId);
        const plant = await this.get(ctx, params.plantId);
...
__decorate([
    ...
    __metadata("design:paramtypes", [fabric_contract_api_1.Context, String]),
    __metadata("design:returntype", Promise)
], PlantOperations.prototype, "addPodtoPlant", null);
__decorate([
    fabric_contract_api_1.Transaction(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [fabric_contract_api_1.Context, String]),
    __metadata("design:returntype", Promise)
], PlantOperations.prototype, "removePodfromPlant", null);
