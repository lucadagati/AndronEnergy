       ...
       let plot=[]
        props.elem.forEach((val,key)=>{
            if(Array.isArray(val)){
                ...
                plot.push( <div className="plot" key={"div"+val[2]} style={{marginLeft:"0"}}>
                    <Plot 
                        className="exchanged_plot"  key={key+val[2]}
                        data={[{x:[1,2,3,4,5,6,7,8],y:[2,3,4,5,6,7,8,9],
                        type:'scatter',
                        mode: 'lines+markers',
                        maker:{color:'#007bff'},
                        }]}
                        layout={ {width: 620 , height: 520,responsive: true,title: props.title_list+ ' '+val[2].replace(/([A-Z])/g, ' $1').trim(),font:{family: 'Courier New, monospace',
                            size: 15},
